...
import jwtValidate from '../middleware/jwt-validate.middleware';
...
router.post('/sign-in', async (req, res) => {
    ...
    // 로그인 성공
    const accessToken = sign({ userId: user.userId }, 'resume@#', { expiresIn: '12h' })
    const refreshToken = sign({ userId: user.userId }, 'resume&%*', { expiresIn: '7d' });
    return res.json({
        accessToken,
        refreshToken,
    })
